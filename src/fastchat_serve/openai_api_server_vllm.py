...
from fastchat_serve.protocol.api_protocol import (
...
def check_requests(request) -> Optional[JSONResponse]:
    ...
    if request.use_beam_search is not None and request.use_beam_search:
        if not math.isclose(request.temperature, 0):
            return create_error_response(
                ErrorCode.PARAM_OUT_OF_RANGE,
                f"When using beam search, {request.temperature} must be 0 - 'temperature'",
            )


        if request.best_of is None or request.best_of < 2:
            return create_error_response(
                ErrorCode.PARAM_OUT_OF_RANGE,
                f"When using beam search, {request.best_of} is less than the minimum of 2 - 'best_of'",
            )
        
        if request.n is None and request.n < 1:
            return create_error_response(
                ErrorCode.PARAM_OUT_OF_RANGE,
                f"When using beam search, {request.n} is less than the minimum of 2 - 'n'",
            )
    
        if request.n is not None and request.best_of is not None and not(request.n == request.best_of):
            return create_error_response(
                ErrorCode.PARAM_OUT_OF_RANGE,
                f"When using beam search, {request.n} is not equal to {request.best_of} - 'n == best_of'",
            )
        
    if request.temperature is not None and math.isclose(request.temperature, 0) and (request.use_beam_search is None or not request.use_beam_search):
        
        if request.best_of is None or request.best_of > 1:
            return create_error_response(
                ErrorCode.PARAM_OUT_OF_RANGE,
                f"When using greedy sampling {request.best_of} must be 1 - 'best_of'",
            )
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    logger.debug(f'chat_completions: {chat_completions}')
    usage = UsageInfo()
    for i, content in enumerate(all_tasks):
        ...
        for choice_index in range(request.n):
            i = content["index"][choice_index]
            choices.append(
                ChatCompletionResponseChoice(
                    index=i,
                    message=ChatMessage(role="assistant", content=content["text"][choice_index]),
                    finish_reason=(content["finish_reason"][choice_index] if content["finish_reason"][choice_index] is not None else "stop")
...
async def chat_completion_stream_generator(
    ...
    async for content in generate_completion_stream(gen_params, worker_addr):
        ...
        logger.debug(f"content: {content}")
        for choice_index, text in enumerate(content["text"]):
            i = content["index"][choice_index]
            if i > n - 1:
                continue
            decoded_unicode = text.replace("\ufffd", "")
            decoded_unicode = text
            delta_text = decoded_unicode[len(previous_texts[i]):]
            previous_texts[i] = decoded_unicode

            choice_data = ChatCompletionResponseStreamChoice(
                index=content["index"][choice_index],
                delta=DeltaMessage(content=delta_text),
                finish_reason=content["finish_reason"][choice_index],
            )
            ...
            if delta_text is None:
                if content.get("finish_reason", None) is not None:
                    ...
                    # finish_stream_events_id.append(i)
                continue
            yield f"data: {chunk.json(exclude_unset=True, ensure_ascii=False)}\n\n"
