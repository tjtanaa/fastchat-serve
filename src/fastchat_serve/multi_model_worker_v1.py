...
from fastchat_serve.model_worker import ModelWorker, worker_id, logger
from fastchat.utils import build_logger, pretty_print_semaphore, get_context_length
from fastchat_serve.utils.config_parser import MultiModelWorkerArgParser
...
def create_multi_model_worker():
    ...
    if args.config:
        args = MultiModelWorkerArgParser(args.config).override(args)
    args.dtype = "half"
    ...
    for conv_template, model_path, model_names in zip(
        ...
        logger.info(f"Create worker for model: {model_names}")
        w = ModelWorker(
